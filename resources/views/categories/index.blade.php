<x-app-layout>
    <x-slot name="header">
        <h2 class="pos-page-title">{{ __('Data Kategori') }}</h2>
        <p class="pos-page-subtitle">MANAGE INVENTORY CATEGORIES AND DEPARTMENTS</p>
    </x-slot>
            ...
            <div class="glass-card-body">
                <div class="section-header">
                    ...
                    <div class="flex items-center gap-2">
                        <form id="bulk-action-form" action="{{ route('categories.bulkDelete') }}" method="POST" style="display: none;">
                            @csrf
                        ...
                        <a href="{{ route('categories.create') }}" class="btn btn-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg> Tambah Kategori
                        </a>
                    </div>
                </div>
                ...
                <div class="pos-table-wrap">
                    <table class="pos-table">
                        ...
                        <tbody>
                            @forelse($categories as $category)
                                <tr>
                                    <td style="text-align: center;">
                                        <input type="checkbox" class="select-item rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500" value="{{ $category->id }}" style="cursor: pointer;">
                                    </td>
                                    <td class="mono text-muted">{{ $loop->iteration }}</td>
                                    <td class="font-medium text-gray-100">{{ $category->nama_kategori }}</td>
                                    <td>
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('categories.edit', $category->id) }}" class="btn btn-edit">
                                                Edit
                                            </a>
                                            <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?');" style="margin: 0;">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-8">Tidak ada kategori ditemukan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            bulkDeleteBtn.addEventListener('click', function () {
                ...
                if (confirm(`Apakah Anda yakin ingin menghapus ${count} kategori terpilih secara massal?`)) {
